...
const srcPath = dirname(require.resolve('@instaql/db-template'));
...
const makePackage = ({ name, description, author }) => {
  return {
    ...
    private: true,
    scripts: {
      test: 'FAST_TEST=1 instaql-db-templatedb && jest',
...
export const initSkitch = async () => {
  ...
  shell.cp('-r', `${srcPath}/instaql/*`, `${dir}/`);
  shell.cp('-r', `${srcPath}/instaql/.*`, `${dir}/`);
